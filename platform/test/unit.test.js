...
test('salesPrompt: tom padrão proíbe gíria; tom informal libera', () => {
  // Regressão: o bloco de estilo autorizava "pra"/"tá"/"beleza" sem olhar o tom,
  // e o agente tratava um diretor de indústria por "beleza, me fala aí".
  const formal = salesPrompt.build({ client: clientePrompt, state: {} });
  assert.ok(formal.includes('acolhedor, educado e atencioso'), 'não aplicou o tom padrão');
  assert.ok(formal.includes('Evite gírias'), 'não proibiu gíria no tom padrão');
  assert.ok(!formal.includes('"beleza" cabem'), 'ainda autoriza gíria por padrão');

  const casual = salesPrompt.build({
    client: { ...clientePrompt, playbook: { ...clientePrompt.playbook, tom: 'informal e descontraído' } },
    state: {}
  });
  assert.ok(casual.includes('"beleza" cabem'), 'tom informal deveria liberar a coloquialidade');
  assert.ok(!casual.includes('Evite gírias'), 'proibiu gíria num tom que pediu informalidade');
});

test('salesPrompt: o tom do cliente aparece no prompt', () => {
  const p = salesPrompt.build({
    client: { ...clientePrompt, playbook: { ...clientePrompt.playbook, tom: 'sóbrio e técnico' } },
    state: {}
  });
  assert.ok(p.includes('sóbrio e técnico'));
});
