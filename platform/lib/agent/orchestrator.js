...
async function loadContext(clientId, phone) {
  const [clientRes, stateRes, leadRes, assetsRes, historyRes] = await Promise.all([
    pool.query('select * from public.clients where client_id = $1', [clientId]),
    pool.query('select * from public.conversation_state where client_id = $1 and phone = $2', [clientId, phone]),
    pool.query('select * from public.leads where client_id = $1 and phone = $2', [clientId, phone]),
    pool.query('select id, name, description, url, mime_type, kind, file_name from public.assets where client_id = $1 and active = true order by id', [clientId]),
    pool.query(
