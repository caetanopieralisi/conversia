...
async function renderAdminNovoCliente() {
  ...
  main.innerHTML = 'Carregando...';
  let templates = [];
  try { templates = await adminRequest('/prompt-templates'); } catch (e) { /* segue sem templates */ }
  ...
  main.innerHTML = `
    ...
    <div class="form-card">
      ...
      <label>Modelo de negócio (aplica um prompt pronto)</label>
      <select id="n-template">
        <option value="">Personalizado (prompt padrão da plataforma)</option>
        ${templates.map((t, i) => `<option value="${i}">${escapeHtml(t.nome)}</option>`).join('')}
      </select>
      <label>Nicho</label>
  ...
  document.getElementById('n-save').onclick = async () => {
    const templateIdx = document.getElementById('n-template').value;
    const body = {
      ...
      system_prompt: templateIdx !== '' ? templates[templateIdx].prompt : undefined
    };
