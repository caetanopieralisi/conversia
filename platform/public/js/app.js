...
function renderLogin() {
  ...
  async function doLogin() {
    ...
    // Se digitar o usuário admin, entra direto no painel administrativo
    if (email.toLowerCase() === 'admin') {
      try {
        const data = await adminRequestLogin(email, password);
...
function renderShell(page) {
  ...
  app.innerHTML = `
    <div class="shell">
      <div class="sidebar">
        ...
        <a data-page="vendedor" class="${page === 'vendedor' ? 'active' : ''}"><span class="ic">🎯</span>Seu vendedor</a>
        ...
        <a data-page="arquivos" class="${page === 'arquivos' ? 'active' : ''}"><span class="ic">📎</span>Arquivos do agente</a>
        <a data-page="integracoes" class="${page === 'integracoes' ? 'active' : ''}"><span class="ic">🔗</span>Integrações</a>
  ...
  // v6 — definidas em app-v6.js
  else if (page === 'vendedor') renderVendedor();
  else if (page === 'arquivos') renderArquivos();
  else if (page === 'integracoes') renderIntegracoes();
  else if (page === 'simulador') renderSimuladorV6();
...
async function loadChat(phone, silent) {
  ...
  try {
    ...
    if (!silent) chat.innerHTML = `<div class="empty-state" style="color:var(--red)">Erro ao carregar conversa: ${escapeHtml(e.message)}</div>`;
    return;
  }

  // Refresh "silencioso" (chamado pelo polling): só atualiza mensagens, notas e o
  // estado de pausa. NUNCA reconstrói o input de mensagem nem o de notas, pra não
  // apagar o que o usuário está digitando no meio da atualização.
  if (silent && document.getElementById('chat-msgs') && chat.dataset.phone === phone) {
    const msgsBox = document.getElementById('chat-msgs');
    const wasNearBottom = msgsBox.scrollTop + msgsBox.clientHeight >= msgsBox.scrollHeight - 60;
    ...
    if (notesBtn) notesBtn.textContent = `📝 Notas (${notes.length})`;
    const notesItems = document.getElementById('notes-items');
    if (notesItems && document.activeElement?.id !== 'note-input') {
      notesItems.innerHTML = notes.map(n => `
      ...
      notesItems.querySelectorAll('[data-noteid]').forEach(b => {
        b.onclick = async () => {
    ...
    if (pauseToggle) pauseToggle.classList.toggle('on', !!pauseState.paused);
    return;
  }

  chat.dataset.phone = phone;
  chat.innerHTML = `
...
// Mesma autenticação de admin, outra base: /api/onboarding é a rota que cria o
// cliente COMPLETO (tenant + login + playbook + follow-ups + instância +
// webhook). O adminRequest prefixa /api/admin e não serve aqui.
async function onboardingRequest(path, opts = {}) {
  const token = localStorage.getItem('adminToken');
...
function renderAdminShell(page, param) {
  app.innerHTML = `
    <div class="shell">
      <div class="sidebar">
        ...
        <a data-page="admin-financeiro" class="${page === 'financeiro' ? 'active' : ''}"><span class="ic">💰</span>Financeiro</a>
        <a data-page="admin-risco" class="${page === 'risco' ? 'active' : ''}"><span class="ic">⚠️</span>Risco de cancelamento</a>
  ...
  else if (page === 'financeiro') renderAdminFinanceiro();
  else if (page === 'risco') renderAdminRisco();
}
  ...
  try {
    ...
    list.innerHTML = clients.map(c => `
      <div class="client-row" data-id="${c.client_id}">
        ...
        <div class="info">
          <div class="n">${escapeHtml(c.nome_empresa || c.client_id)} ${c.alertas_abertos > 0 ? `<span class="badge fechado" style="margin-left:6px">${c.alertas_abertos} alerta(s)</span>` : ''} ${c.payment_status === 'atrasado' ? `<span class="badge fechado" style="margin-left:6px">mensalidade atrasada</span>` : ''}</div>
          <div class="s">${escapeHtml(c.nicho || '-')} · ${c.active ? 'Ativo' : 'Pausado'} · plano: ${escapeHtml(c.plan || 'trial')}${c.monthly_fee > 0 ? ` · ${fmtMoney(c.monthly_fee)}/mês` : ''}</div>
        </div>
        <div class="stats">
...
async function renderAdminOverview() {
  ...
  try {
    ...
    <div class="cards">
      <div class="card"><div class="label">MRR (receita mensal)</div><div class="value">${fmtMoney(o.mrr)}</div></div>
...
async function renderAdminFinanceiro() {
  ...
  main.innerHTML = 'Carregando...';
  try {
    const f = await adminRequest('/finance');
    main.innerHTML = `
    <div class="page-title">Financeiro</div>
    <div class="page-sub">Sua receita como dono da ConversIA (o que os clientes te pagam)</div>
    <div class="cards">
      <div class="card"><div class="label">MRR atual</div><div class="value">${fmtMoney(f.mrr_atual)}</div></div>
      <div class="card"><div class="label">Clientes pagantes</div><div class="value">${f.clientes_pagantes}</div></div>
      <div class="card"><div class="label">Mensalidades atrasadas</div><div class="value" style="color:${f.clientes_atrasados > 0 ? 'var(--red)' : 'inherit'}">${f.clientes_atrasados}</div></div>
    </div>
    <div class="panel-box" style="padding:18px;margin-bottom:24px">
      <div style="font-size:14px;font-weight:600;margin-bottom:12px">Receita coletada por mês</div>
      <canvas id="chart-receita-mes" height="90"></canvas>
    </div>
    <div class="panel-box" style="padding:18px">
      <div style="font-size:14px;font-weight:600;margin-bottom:12px">Novos clientes por mês</div>
      <canvas id="chart-clientes-mes" height="90"></canvas>
    </div>`;

    if (typeof Chart === 'undefined') {
      throw new Error('Biblioteca de gráficos não carregou. Recarregue a página (Ctrl+F5) ou verifique se alguma extensão do navegador está bloqueando scripts.');
    }
    const chartOpts = {
      responsive: true,
      scales: { y: { ticks: { color: '#8b93a7' }, grid: { color: '#2a2f3c' } }, x: { ticks: { color: '#8b93a7' }, grid: { display: false } } },
      plugins: { legend: { display: false } }
    };
    new Chart(document.getElementById('chart-receita-mes'), {
      type: 'bar',
      data: { labels: f.receita_coletada_por_mes.map(r => r.mes), datasets: [{ data: f.receita_coletada_por_mes.map(r => r.total), backgroundColor: '#6c5ce7' }] },
      options: chartOpts
    });
    new Chart(document.getElementById('chart-clientes-mes'), {
      type: 'bar',
      data: { labels: f.novos_clientes_por_mes.map(r => r.mes), datasets: [{ data: f.novos_clientes_por_mes.map(r => r.total), backgroundColor: '#4f7cff' }] },
      options: chartOpts
    });
  } catch (e) {
    main.innerHTML = `<div style="color:var(--red)">Erro ao carregar financeiro: ${escapeHtml(e.message)}</div>`;
  }
}

// ---------- ADMIN: RISCO DE CANCELAMENTO ----------
async function renderAdminRisco() {
  const main = document.getElementById('main');
  main.innerHTML = 'Carregando...';
  try {
    const list = await adminRequest('/risk');
    const cores = { alto: 'fechado', medio: 'aguardando_humano', baixo: 'ativo' };
    main.innerHTML = `
    <div class="page-title">Radar de risco de cancelamento</div>
    <div class="page-sub">Clientes que podem estar prestes a cancelar — aja antes de perder</div>
    <div id="risk-list">
      ${list.map(c => `
        <div class="client-row" data-id="${c.client_id}" style="cursor:pointer">
          <div class="avatar">${initials(c.nome_empresa || c.client_id)}</div>
          <div class="info">
            <div class="n">${escapeHtml(c.nome_empresa || c.client_id)} <span class="badge ${cores[c.risco]}" style="margin-left:6px">risco ${c.risco}</span></div>
            <div class="s">${c.motivos.length ? escapeHtml(c.motivos.join(' · ')) : 'Sem sinais de risco'}</div>
          </div>
        </div>`).join('') || '<div style="color:var(--muted)">Nenhum cliente ativo pra avaliar ainda</div>'}
    </div>`;
    main.querySelectorAll('#risk-list [data-id]').forEach(row => {
      row.onclick = () => { location.hash = 'admin-client-' + row.dataset.id; };
    });
  } catch (e) {
    main.innerHTML = `<div style="color:var(--red)">Erro ao carregar radar de risco: ${escapeHtml(e.message)}</div>`;
  }
}
...
async function renderAdminClientEdit(clientId) {
  const main = document.getElementById('main');
  main.innerHTML = 'Carregando...';
  let c, alerts, weekly, checklist;
  try {
    [c, alerts, weekly, checklist] = await Promise.all([
      ...
      adminRequest('/clients/' + encodeURIComponent(clientId) + '/checklist')
    ]);
  } catch (e) {
  ...
  const checklistLabels = {
    instancia_configurada: 'Instância do WhatsApp conectada',
    prompt_personalizado: 'Prompt do agente personalizado',
    telefone_dono_cadastrado: 'Telefone do dono cadastrado',
    cobranca_configurada: 'Mensalidade configurada',
    primeiro_lead_capturado: 'Primeiro lead capturado',
    primeira_mensagem_enviada: 'Primeira mensagem enviada pelo agente',
    base_conhecimento: 'Base de conhecimento enviada'
  };
  ...
  main.innerHTML = `
    ...
    <div class="panel-box" style="padding:16px;margin-bottom:20px">
      <div style="display:flex;justify-content:space-between;font-size:13px;margin-bottom:8px">
        <span style="font-weight:600">Checklist de ativação</span><span style="color:var(--muted)">${checklistDone}/${checklistTotal}</span>
      </div>
      ${Object.entries(checklistLabels).map(([key, label]) => `
        <div style="display:flex;align-items:center;gap:8px;font-size:13px;padding:4px 0;color:${checklist[key] ? 'var(--text)' : 'var(--muted)'}">
          <span>${checklist[key] ? '✅' : '⬜'}</span><span>${label}</span>
        </div>`).join('')}
    </div>
    ...
    <div class="page-title" style="margin-top:28px;font-size:16px">Cobrança</div>
    <div class="form-card">
      <label>Telefone do dono (WhatsApp, com DDI)</label>
      <input id="c-phone" value="${escapeHtml(c.owner_phone || '')}" placeholder="5511999999999" />
      <label>Mensalidade (R$)</label>
      <input id="c-fee" type="number" step="0.01" value="${c.monthly_fee || ''}" />
      <label>Dia de vencimento</label>
      <input id="c-billing-day" type="number" min="1" max="28" value="${c.billing_day || 5}" />
      <div style="font-size:13px;color:var(--muted);margin-top:10px">
        Status: <span class="badge ${c.payment_status === 'atrasado' ? 'fechado' : c.payment_status === 'pendente' ? 'aguardando_humano' : 'ativo'}">${c.payment_status || 'em_dia'}</span>
        ${c.next_due_date ? ` · vencimento: ${new Date(c.next_due_date).toLocaleDateString('pt-BR')}` : ''}
      </div>
      <div class="save-row">
        <button id="c-billing-save">Salvar cobrança</button>
        <button id="c-mark-paid">Marcar como pago</button>
        ${c.owner_phone ? `<button class="ghost" id="c-cobrar-whatsapp">Cobrar via WhatsApp</button>` : ''}
        <span class="saved-msg" id="c-billing-saved" style="display:none">Salvo ✓</span>
      </div>
    </div>
  ...
  document.getElementById('c-billing-save').onclick = async () => {
    await adminRequest('/clients/' + encodeURIComponent(clientId), {
      method: 'PUT',
      body: {
        owner_phone: document.getElementById('c-phone').value.trim(),
        monthly_fee: parseFloat(document.getElementById('c-fee').value) || 0,
        billing_day: parseInt(document.getElementById('c-billing-day').value) || 5
      }
    });
    const saved = document.getElementById('c-billing-saved');
    saved.style.display = 'inline';
    setTimeout(() => (saved.style.display = 'none'), 2000);
    renderAdminClientEdit(clientId);
  };
  document.getElementById('c-mark-paid').onclick = async () => {
    await adminRequest('/clients/' + encodeURIComponent(clientId) + '/mark-paid', { method: 'POST' });
    renderAdminClientEdit(clientId);
  };
  const cobrarBtn = document.getElementById('c-cobrar-whatsapp');
  if (cobrarBtn) cobrarBtn.onclick = () => {
    const fee = document.getElementById('c-fee').value || c.monthly_fee || '';
    const texto = encodeURIComponent(`Olá! Passando pra lembrar da mensalidade da ConversIA${fee ? ` no valor de R$ ${fee}` : ''}. Qualquer dúvida, me chama por aqui 🙂`);
    window.open(`https://wa.me/${document.getElementById('c-phone').value.replace(/\D/g, '')}?text=${texto}`, '_blank');
  };
}
...
async function renderAdminNovoCliente() {
  const main = document.getElementById('main');
  main.innerHTML = 'Carregando...';
  // Templates de PLAYBOOK (não só de prompt): é o que configura o vendedor.
  let templates = [];
  try { templates = await onboardingRequest('/templates'); } catch (e) { /* segue sem templates */ }

  main.innerHTML = `
    <div class="page-title">Novo cliente</div>
    <div class="page-sub">Cria a empresa, o login do dono, o playbook, os follow-ups e aponta o webhook da Evolution — de uma vez</div>
    <div class="form-card">
      ...
      <label>Modelo de negócio (aplica o playbook de vendas pronto)</label>
      <select id="n-template">
        <option value="">Personalizado (prompt padrão da plataforma)</option>
        ${templates.map(t => `<option value="${escapeHtml(t.slug)}">${escapeHtml(t.nome)}</option>`).join('')}
      </select>
      <label>Nicho</label>
      <input id="n-nicho" placeholder="Alimentação" />
      <label>Instância Evolution API</label>
      <input id="n-instance" placeholder="pizzaria-do-joao" />
      <div class="hint" style="margin:-6px 0 10px;font-size:12px;opacity:.7">
        Se a instância já existe e já está com o WhatsApp conectado, escreva o nome dela aqui:
        a plataforma reaproveita a conexão e só reaponta o webhook — não pede QR de novo.
      </div>
      <label>Telefone do dono (WhatsApp, com DDI) — usado pra você mandar lembrete de cobrança</label>
      <input id="n-phone" placeholder="5511999999999" />
      <label>Mensalidade cobrada (R$)</label>
      <input id="n-fee" type="number" step="0.01" placeholder="Ex: 297" />
      <label>Dia de vencimento</label>
      <input id="n-billing-day" type="number" min="1" max="28" placeholder="Ex: 5" value="5" />
      <label>Plano</label>
      <select id="n-plan">
  ...
  document.getElementById('n-save').onclick = async () => {
    const body = {
      ...
      evolution_instance: document.getElementById('n-instance').value.trim() || undefined,
      owner_phone: document.getElementById('n-phone').value.trim(),
      monthly_fee: parseFloat(document.getElementById('n-fee').value) || 0,
      billing_day: parseInt(document.getElementById('n-billing-day').value) || 5,
      plan: document.getElementById('n-plan').value,
      owner_email: document.getElementById('n-email').value.trim(),
      template_slug: document.getElementById('n-template').value || undefined
    };

    const resultBox = document.getElementById('n-result');
    const botao = document.getElementById('n-save');
    botao.disabled = true;
    resultBox.style.color = '';
    resultBox.textContent = 'Criando cliente, instância e webhook...';

    try {
      // /api/onboarding, e não /api/admin/clients: só esta rota gera o
      // inbound_token e aponta o webhook da Evolution. Sem ela o cliente é
      // criado no banco e fica MUDO — nada chega.
      const d = await onboardingRequest('/', { method: 'POST', body });

      const passosHtml = (d.passos || [])
        .map(p => `<li>${p.ok ? '✓' : '✗'} ${escapeHtml(p.passo)}${
          p.detalhe ? ' — ' + escapeHtml(String(p.detalhe)) : ''}${
          p.erro ? ' — <span style="color:var(--red)">' + escapeHtml(String(p.erro)) + '</span>' : ''}</li>`)
        .join('');

      const qr = d.whatsapp && d.whatsapp.qrcode;
      const qrHtml = qr
        ? `<div style="margin-top:14px">
             <div style="margin-bottom:6px">Escaneie no WhatsApp do cliente: <b>Aparelhos conectados → Conectar aparelho</b></div>
             <img src="${qr.startsWith('data:') ? qr : 'data:image/png;base64,' + qr}"
                  alt="QR code" style="width:260px;height:260px;background:#fff;padding:8px;border-radius:8px" />
           </div>`
        : `<div style="margin-top:10px;opacity:.8">
             Sem QR — a instância <b>${escapeHtml((d.whatsapp && d.whatsapp.instancia) || '')}</b>
             já estava conectada. O webhook foi reapontado e ela continua ligada.
           </div>`;

      resultBox.innerHTML = `
        <div style="color:var(--green);font-weight:600">Cliente criado.</div>
        <div style="margin-top:6px">Login: <b>${escapeHtml(d.login.email)}</b> ·
          Senha temporária: <b>${escapeHtml(d.login.senha_temporaria)}</b></div>
        <div style="margin-top:6px;font-size:12px;opacity:.75">
          Webhook: <code>${escapeHtml((d.whatsapp && d.whatsapp.webhook_url) || '')}</code></div>
        <ul style="margin-top:10px;line-height:1.7">${passosHtml}</ul>
        ${qrHtml}`;
    } catch (e) {
...
// Renderiza só depois que TODOS os scripts carregaram: as telas da v6 vivem em
// app-v6.js, carregado depois deste arquivo. Chamar render() direto aqui
// quebraria um acesso direto a #vendedor / #arquivos / #integracoes.
